...
private:
    ...
    geometry_msgs::Pose getHeadPoseByTag(std::string tag);
    ...
    void addHeadPosesToBuffer(std::vector<geometry_msgs::Pose> headPoseArray, uint32_t index);
    ...
    std::map<uint32_t, std::vector<geometry_msgs::Pose>> headPoseBuffer;
